  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error('Erro ao buscar MEU usuário (logado):', err)
        if (axios.isAxiosError(err) && err.response?.status === 401) {
          navigate('/login')
          return
        }
        setError('Não foi possível carregar o ID do usuário logado.')
        setLoading(false)
        return
      }
      ...
      if (profileToFetchId) {
        try {
          ...
          setError('Perfil não encontrado ou erro ao carregar dados.')
          setUserData(null)
          setLoading(false)
          return
        }
      } else {
        setError('Nenhum perfil especificado para visualização.')
        setLoading(false)
        return
      }
